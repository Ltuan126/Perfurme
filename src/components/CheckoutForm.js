  ...
  return (
    <form onSubmit={handleSubmit} className="glass mx-auto mt-4 p-6 max-w-sm">
      ...
      <div className="mb-3 text-sm text-slate-700 bg-blue-50 border border-blue-100 rounded-2xl px-4 py-3">
        <div className="flex justify-between"><span>Tạm tính</span><span className="font-semibold">{summary.subtotal.toLocaleString('vi-VN')}₫</span></div>
        <div className="flex justify-between"><span>Giảm combo 15ml{summary.miniCount >= 3 ? '' : ' (chọn đủ 3 chai 15ml để nhận -10%)'}</span><span className="font-semibold text-green-600">-{summary.discount.toLocaleString('vi-VN')}₫</span></div>
        <div className="flex justify-between text-blue-700 font-bold mt-1"><span>Thành tiền</span><span>{summary.total.toLocaleString('vi-VN')}₫</span></div>
        <div className="text-xs text-slate-500 mt-1">Ước tính nhận <span className="font-semibold text-slate-700">{summary.estPoints}</span> điểm (1 điểm mỗi 10.000₫).</div>
      </div>
